...
如果 Chrome 对某个本地视频出现 0:00、黑屏或“无法解码”，可以使用压缩包内的 `启动系统播放器助手.bat`。它会在本机启动一个只监听 `127.0.0.1` 的小助手，并让网站把本地视频交给 Windows 默认播放器打开，绕开 Chrome 的视频解码链路。助手只允许打开启动时选择的本地云盘目录及其子目录，不上传文件。
