...
如果浏览器不能直接解码上传的视频（例如部分 HEVC/H.265、MOV/手机视频），网页播放助手会尝试在当前设备本机使用 ffmpeg.js 将其转换为 MP4（H.264 + AAC）后播放。原视频不会上传到服务器。首次自动转换会加载较大的浏览器转码组件并消耗设备 CPU/内存；转换结果会缓存在浏览器 IndexedDB 中，后续点击同一视频可直接播放。
